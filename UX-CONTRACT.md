...
- Campaign-creation route policy: Once an advertiser selects a screen and enters booking, the buying-step indicator is display-only and sidebar/workspace navigation is locked. Browser Back remains on booking and page unload warns about the active draft. The flow releases navigation only after the campaign request succeeds or the advertiser explicitly cancels; cancellation discards the booking draft and returns to screen discovery.

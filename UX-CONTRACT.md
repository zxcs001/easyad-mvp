...
| Cancel campaign | `Cancel campaign` in the booking step | none | Screen discovery | none | discards the in-progress booking draft | discovery heading | current task decision |
...
- Campaign-creation route policy: The advertiser creation path is strictly `Find screens` → `Request dates` → `Make an ad`. `Request dates` and `Make an ad` are internal steps and are omitted from the sidebar. A direct booking URL returns to screen discovery; a direct creative URL is valid only when it names an accessible existing booking and then behaves as editing, without the creation stepper. After an advertiser selects a screen, the buying-step indicator is display-only and sidebar/workspace/Back navigation remains locked to the current step. A successful date request advances to the newly created campaign's locked creative step. Navigation is released only after a server response verifies that the submitted creative belongs to that campaign, or when the advertiser cancels during the date-request step; cancellation discards the booking draft and returns to screen discovery. Page unload warns throughout the locked flow.
- Cookie-control route policy: The cookie choices panel and its persistent `Cookie settings` trigger render only on the root 3D starter and portal (`/` with no `view`, or `view=portal`). Authenticated workspaces, discovery, login, government, player, and detail routes do not render either control.
